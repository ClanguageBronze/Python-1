...
def replyAptData(date_param, user, loc_param='11710'):
    res_list = Telegramnoti.getAreaData( loc_param, date_param )
    msg = ''
    for r in res_list:
    ...
    if msg:
        ...
        Telegramnoti.sendMessage( user, '%s 지역에 해당하는 데이터가 없습니다.'%date_param )

def replyPeriodData(date_param, user, loc_param='11710'):
    res_list = Telegramnoti.getPeriodData( loc_param, date_param )
    msg = ''
    for r in res_list:
        if len(r+msg)+1>Telegramnoti.MAX_MSG_LENGTH:
            Telegramnoti.sendMessage( user, msg )
...
def handle(msg):
    ...
    if text.startswith('지역') and len(args)>1:
        ...
        replyPeriodData( args[2], chat_id, args[1] )
    else:
